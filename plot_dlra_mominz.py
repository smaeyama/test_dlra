...
"""Animate the time evolution of |phi(z)| from lingk_dlra.nc."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Animate |phi(z)| from lingk_dlra.nc")
...
def load_dlra_mominz(path: str | Path) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
...
def save_frames(
    ...
    for iframe, (time_value, frame) in enumerate(zip(selected_times, selected_phi_abs)):
        ...
        ax.plot(z, frame, color="tab:orange", lw=2)
        ax.set_xlabel("z")
        ax.set_ylabel("|phi(z)|")
        ax.set_yscale("log")
        ...
        fig.tight_layout()
        fig.savefig(frames_dir / f"dlra_mominz_{iframe:04d}.png", dpi=160)
        plt.close(fig)


def main() -> None:
    args = parse_args()
    time, z, phi_abs = load_dlra_mominz(args.input)
    ...
    if args.frames_dir is not None:
        ...
        print(f"[plot_dlra_mominz] wrote frames: {args.frames_dir}")
    ...
    if args.save is not None:
        ...
        print(f"[plot_dlra_mominz] wrote animation: {save_path}")
