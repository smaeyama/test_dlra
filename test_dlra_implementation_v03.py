...
class LowRankApprox:
    ...
    def init_from_full(self, f):
        ...
        # 重み付き直交化（Ensignと同じ）
        self.X[:, :r] = X / np.sqrt(dx)
        self.S[:r, :r] = np.sqrt(dx) * S * np.sqrt(dv)
        self.V[:, :r] = V / np.sqrt(dv)
...
def K_step_RK4(f_lra, E, dt):
    ...
    dVdv = (-np.roll(V, 2, axis=0) + 8*np.roll(V, 1, axis=0) - 8*np.roll(V, -1, axis=0) + np.roll(V, -2, axis=0)) / (12.0 * dv)
    ...
    f_lra.X = Q / np.sqrt(dx)  # (Nx,Nr)
    f_lra.S = R * np.sqrt(dx)  # (Nr,Nr)
    return f_lra
...
def K_rhs(K, C1, C2, E):
    ...
    term_x = - dKdx @ C1.T             # (Nx, r)
    term_E = (E[:, None] * K) @ C2.T   # (Nx, r)
...
def S_rhs(S, C1, C2, D1, D2):
    ...
    Right-hand side of S-step ODE:
        dS/dt = D2 @ S @ C1.T - D1 @ S @ C2.T
    """
    return D2 @ S @ C1.T - D1 @ S @ C2.T
...
def L_step_RK4(f_lra, E, dt):
    ...
    f_lra.V = Q / np.sqrt(dv)    # (Nv,Nr)
    f_lra.S = R.T * np.sqrt(dv) # (Nr,Nr) Note that L = V S^T.
...
def L_rhs(L, D1, D2):
    ...
    dLdv = (-np.roll(L, 2, axis=0) + 8*np.roll(L, 1, axis=0) - 8*np.roll(L, -1, axis=0) + np.roll(L, -2, axis=0)) / (12.0 * dv)
    term_v = dLdv @ D1.T
    term_x = (v[:, None] * L) @ D2.T
