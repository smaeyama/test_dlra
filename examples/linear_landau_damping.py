...
"""Run the existing scripts for a linear Landau damping example."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the linear Landau damping example with the existing scripts")
    parser.add_argument("--outdir", type=Path, default=REPO_ROOT / "examples" / "output" / "linear_landau")
    ...
    parser.add_argument("--lx", type=float, default=10.0 * math.pi)
    parser.add_argument("--lv", type=float, default=5.0)
...
def estimate_recurrence_time(lx: float, lv: float, nv: int) -> float:
    dv = 2.0 * lv / (nv - 1)
    wave_number = 4.0 * math.pi / lx
    return 2.0 * math.pi / (wave_number * dv)


def main() -> None:
    args = parse_args()
    args.outdir.mkdir(parents=True, exist_ok=True)
    recurrence_time = estimate_recurrence_time(args.lx, args.lv, args.nv)
    ...
    figure_path = args.outdir / "linear_landau_summary.png"
    ...
    print(f"Estimated recurrence time: {recurrence_time:.3f}")
