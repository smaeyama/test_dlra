...
def weighted_gram(matrix: np.ndarray, weight: float) -> np.ndarray:
    return matrix.conj().T @ matrix * weight


def complex_to_parts(array: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    return np.real(array), np.imag(array)
...
def _vintegral_species(wf: np.ndarray, geom: GKGeometry) -> np.ndarray:
    ...
    wn = np.sum(weighted, axis=(1, 2)) * (2.0 * PI * geom.dv * geom.dvp[:, None])
...
def solve_magnetic_field_from_f(fk: np.ndarray, geom: GKGeometry) -> np.ndarray:
    ...
    with np.errstate(divide="ignore", invalid="ignore"):
        return np.where(geom.ksq != 0.0, nk * geom.params.beta / geom.ksq, 0.0)
...
def _extend_state(field: np.ndarray, geom: GKGeometry) -> np.ndarray:
    ...
    v0 = geom.params.nvb
    m0 = geom.params.nvb
    ext[z0 : z0 + geom.nz_tot, v0 : v0 + geom.nv_tot, m0 : m0 + geom.nm_tot, :] = field

    pos = geom.vl > 0.0
    neg = ~pos
    src = slice(m0, m0 + geom.nm_tot)
    if np.any(pos):
        cols = np.flatnonzero(pos) + v0
        ext[z0 + geom.nz_tot, cols, src, :] = ext[z0 + geom.nz_tot - 1, cols, src, :]
        ext[z0 + geom.nz_tot + 1, cols, src, :] = -ext[z0 + geom.nz_tot - 2, cols, src, :] + 2.0 * ext[
            z0 + geom.nz_tot - 1, cols, src, :
        ]
    if np.any(neg):
        cols = np.flatnonzero(neg) + v0
        ext[z0 - 1, cols, src, :] = ext[z0, cols, src, :]
        ext[z0 - 2, cols, src, :] = -ext[z0 + 1, cols, src, :] + 2.0 * ext[z0, cols, src, :]
    return ext
...
def rhs_h(
    ...
    for ispec in range(geom.params.ns):
        ...
        dpsidz = (
            -np.roll(psi[..., ispec], -2, axis=0)
            + 8.0 * np.roll(psi[..., ispec], -1, axis=0)
            - 8.0 * np.roll(psi[..., ispec], 1, axis=0)
            + np.roll(psi[..., ispec], 2, axis=0)
        ) / (12.0 * geom.dpara[:, None, None])

        dh[..., ispec] -= geom.vl[None, :, None] * np.sqrt(geom.params.tau[ispec] / geom.params.Anum[ispec]) * dfdz
        dh[..., ispec] += geom.mir[:, None, :] * np.sqrt(geom.params.tau[ispec] / geom.params.Anum[ispec]) * dfdv
        dh[..., ispec] -= (
            cs1
            * geom.fmx
            * geom.vl[None, :, None]
